...
macro_rules! create_structs {
    ($read_func:ident, $write_func:ident, $bytes:expr, $typename:ident) => {
        paste::item! {
            #[doc = "A wrapper for an `audio` buffer of bytes containing `" $typename "` samples stored as `[u8; " $bytes "]`"]
            pub struct [< Audio $typename >]<U, V> {
...
macro_rules! impl_traits {
    ($read_func:ident, $write_func:ident, $bytes:expr, $typename:ident) => {
        paste::item! {
            ...
            impl<'a, T> [< Audio $typename >]<&'a mut Buf<Sample=[u8; $bytes]>, T>
            where
                T: 'a,
            {
                #[doc = "Create a new wrapper for a slice of bytes,"]
            ...
            impl<'a, T> Indirect<'a, T> for [< Audio $typename >]<&'a [u8], T>
            where
                T: Sample<T> + 'a,
            {
                unsafe fn read_sample_unchecked(&self, channel: usize, frame: usize) -> T {
                    T::$read_func(self.buf.get(channel).unwrap().get(frame).unwrap())
                }
            ...
            impl<'a, T> IndirectMut<'a, T> for [< Audio $typename >]<&'a mut [u8], T>
            where
                T: Sample<T> + Clone + 'a,
            {
...
impl_traits!(from_s16_le, to_s16_le, 2, S16LE);
impl_traits!(from_s16_be, to_s16_be, 2, S16BE);
impl_traits!(from_s24_3_le, to_s24_3_le, 3, S24LE3);
impl_traits!(from_s24_3_be, to_s24_3_be, 3, S24BE3);
impl_traits!(from_s24_4_le, to_s24_4_le, 4, S24LE4);
impl_traits!(from_s24_4_be, to_s24_4_be, 4, S24BE4);
impl_traits!(from_s32_le, to_s32_le, 4, S32LE);
impl_traits!(from_s32_be, to_s32_be, 4, S32BE);
impl_traits!(from_f32_le, to_f32_le, 4, F32LE);
impl_traits!(from_f32_be, to_f32_be, 4, F32BE);
impl_traits!(from_f64_le, to_f64_le, 8, F64LE);
impl_traits!(from_f64_be, to_f64_be, 8, F64BE);
