...
mod tests {
    use super::*;
    use audioadapter_sample::sample::{I8, I16_LE, I24_LE, U8};
    ...
    #[test]
    fn read_i8_bytes_interleaved() {
        let data: [u8; 6] = [0, 128, 64, 192, 32, 224];
        let buffer = InterleavedNumbers::<&[I8], f32>::new_from_bytes(&data, 2, 3).unwrap();
        assert_eq!(buffer.read_sample(0, 0).unwrap(), 0.0);
        assert_eq!(buffer.read_sample(1, 0).unwrap(), -1.0);
        assert_eq!(buffer.read_sample(0, 1).unwrap(), 0.5);
        assert_eq!(buffer.read_sample(1, 1).unwrap(), -0.5);
    ...
    #[test]
    fn write_i8_bytes_interleaved() {
        let expected: [u8; 6] = [0, 128, 64, 192, 32, 224];
        let mut data = [0; 6];
        let mut buffer =
            InterleavedNumbers::<&mut [I8], f32>::new_from_bytes_mut(&mut data, 2, 3).unwrap();
        buffer.write_sample(0, 0, &0.0).unwrap();
        buffer.write_sample(1, 0, &-1.0).unwrap();
        buffer.write_sample(0, 1, &0.5).unwrap();
        buffer.write_sample(1, 1, &-0.5).unwrap();
    ...
    #[test]
    fn read_u8_bytes_interleaved() {
        // Unsigned samples are centered at 128.
        let data: [u8; 6] = [128, 0, 192, 64, 160, 96];
        let buffer = InterleavedNumbers::<&[U8], f32>::new_from_bytes(&data, 2, 3).unwrap();
        assert_eq!(buffer.read_sample(0, 0).unwrap(), 0.0);
        assert_eq!(buffer.read_sample(1, 0).unwrap(), -1.0);
        assert_eq!(buffer.read_sample(0, 1).unwrap(), 0.5);
        assert_eq!(buffer.read_sample(1, 1).unwrap(), -0.5);
    ...
    #[test]
    fn write_u8_bytes_interleaved() {
        let expected: [u8; 6] = [128, 0, 192, 64, 160, 96];
        let mut data = [0; 6];
        let mut buffer =
            InterleavedNumbers::<&mut [U8], f32>::new_from_bytes_mut(&mut data, 2, 3).unwrap();
        buffer.write_sample(0, 0, &0.0).unwrap();
        buffer.write_sample(1, 0, &-1.0).unwrap();
        buffer.write_sample(0, 1, &0.5).unwrap();
        buffer.write_sample(1, 1, &-0.5).unwrap();
